 ...
 * has a small, and `--` is free of Markdown entirely. All three are free of
 * Obsidian: strikethrough takes two tildes, a lone caret only means anything at
 * the end of a line, where it names a block, and a rule takes three dashes on a
 * line of their own.
 *
 ...
 * is what keeps a delimiter written as itself — `a ~ b ~ c`, the ASCII em dash
 * in `a -- b -- c` — from opening one; and a run may not hold its own
 * delimiter, which is what closes `~um~ e ~dois~` twice rather than once. The
 * tilde is barred from touching another tilde besides, so `~~riscado~~` stays
 * strikethrough.
 *
