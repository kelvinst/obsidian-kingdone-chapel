...
export class VersionSuggestModal extends SuggestModal<VersionItem> {
  ...
  constructor(app: App, plugin: KingdoneChapelPlugin, loc: Location, items: VersionItem[]) {
    super(app);
    this.plugin = plugin;
    this.loc = loc;
    this.items = items;
    this.setPlaceholder(`${loc.book} ${loc.chapter}${loc.verse ? '.' + loc.verse : ''} — pick a version`);
  ...
  getSuggestions(query: string): VersionItem[] {
    ...
    return this.items.filter((i) => i.label.toLowerCase().includes(q) || i.version.toLowerCase().includes(q));
  }

  renderSuggestion(item: VersionItem, el: HTMLElement) {
    el.createEl('div', { text: item.label, cls: 'kcp-version' });
    if (item.text) el.createEl('small', { text: item.text, cls: 'kcp-preview' });
  }
