...
describe('renderMarks', () => {
  ...
  it('shrinks a run written between commas', () => {
  ...
  it('keeps a single comma inside a run', () => {
    const el = rendered('<p>,,Uma nota, e outra,,</p>');
    expect(marks(el)).toEqual(['span:Uma nota, e outra']);
  });

  it('leaves a decimal comma alone', () => {
    const el = rendered('<p>Custa 1,5 e depois 2,3 reais.</p>');
    expect(marks(el)).toEqual([]);
  });

  it('leaves a sentence whose clauses only end in commas alone', () => {
