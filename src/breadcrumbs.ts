...
export class Breadcrumbs {
  ...
  versionMenu(anchor: HTMLElement, view: MarkdownView, loc: Location, evt: MouseEvent) {
    this.open(anchor, 'kcp-crumb-list', (body, close) => {
  ...
  chapterMenu(anchor: HTMLElement, view: MarkdownView, loc: Location, evt: MouseEvent) {
    this.open(anchor, 'kcp-crumb-grid', (body, close) => {
...
class CrumbMenu {
  ...
  constructor(anchor: HTMLElement, cls: string, onClose: (from: HTMLElement | null) => void) {
    ...
    this.el = document.body.createDiv({ cls: `kcp-crumb-menu ${cls}` });
    this.el.style.setProperty('--kcp-crumb-columns', String(CHAPTERS_PER_ROW));
  ...
  place() {
    ...
    // Long books open scrolled to where the reader already is.
